...
class Zoo: 
    ...
    def remove_enclosure(self, i: int):
        if 0 <= i < len(self._enclosures):
            enclosure_to_remove = self._enclosures.pop(i)
            enclosure_id_to_remove = enclosure_to_remove.get_id()
            for exposition in self._expositions:
                exposition.remove_enclosure_by_id(enclosure_id_to_remove)
    ...
    def clear_vet_logs(self):
        self._vet_logs.clear()
